...
class AudioQueueSource(
    ...
    val sampleOffset: Long
        get() {
            context.makeCurrent()
            val o = bufferOffset + alGetSourcei(source, AL11.AL_SAMPLE_OFFSET).also { checkALError() }
            return o
        }
        ...
        playThread = thread(isDaemon = true) {
            ...
            while (true) {
                ...
                if (queued == 0) {
                    ...
                    val observation = alGetSourcei(source, AL_SOURCE_STATE) == AL_PLAYING
                ...
                val buffersProcessed = alGetSourcei(source, AL_BUFFERS_PROCESSED)
                ...
                for (i in 0 until buffersProcessed) {
                    val unqueue = alSourceUnqueueBuffers(source)
